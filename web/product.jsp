...
<html lang="en">
	...
	<body>
				...
				<div class="row">
					...
					<div class="col-md-5">
						<div class="product-details">
							<h2 class="product-name">${requestScope.data.name}</h2>
							<div class="product-body">
								<c:if test="${requestScope.data.sale_off != 0}">
                                                                    <h3 class="product-price"><fmt:setLocale value = "en_US"/><fmt:formatNumber value = "${requestScope.data.price * (100 - requestScope.data.sale_off) / 100}" type = "currency"/>
                                                                        <del class="product-old-price"><fmt:setLocale value = "en_US"/><fmt:formatNumber value = "${requestScope.data.price}" type = "currency"/></del>
                                                                    </h3>
                                                                </c:if>
                                                                <c:if test="${requestScope.data.sale_off == 0}">
                                                                     <h3 class="product-price"><fmt:setLocale value = "en_US"/><fmt:formatNumber value = "${requestScope.data.price}" type = "currency"/></h3>
                                                                </c:if>
                                                                <c:if test="${requestScope.data.quantity > 0}">
							...
							<c:if test="${sessionScope.account != null}">
                                                            <div class="add-to-cart">
                                                                <button class="add-to-cart-btn" onclick="cartx(${requestScope.pid})"><i class="fa fa-shopping-cart"></i> add to cart</button>
                                                            </div>                            
                                                        </c:if>
                                                        <c:if test="${sessionScope.account == null}">
                                                            <div class="add-to-cart">
							...
							<ul class="product-links">
								<li>Category:</li>
                                                                <li><a href="${requestScope.cat.cname}s">${requestScope.cat.cname}</a></li>
                                                                <li><a href="#">${requestScope.data.subCat.scname}</a></li>
							</ul>
		...
		<script src="js/main.js"></script>
                <script>
                    function cartx(id) {
                        document.x.action="product?pid=${requestScope.pid}&id=" + id;
                        document.x.submit();
                    }
                    
                    function noticex() {
                        alert("You must login to 'Add to cart'");
                        document.x.action="product";
                ...
                <script>
                    function carty(id) {
                        document.y.action="product?pid=${requestScope.pid}&id=" + id;
                        document.y.submit();
                    }
                    
                    function noticey() {
                        alert("You must login to 'Add to cart'");
                        document.y.action="product";
